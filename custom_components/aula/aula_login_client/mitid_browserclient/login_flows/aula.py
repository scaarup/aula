...
if 'Location' in post_broker_response.headers:
    ...
    _LOGGER.debug(f"After-post-broker response content length: {len(after_response.text)}")
    ...
    if saml_form:
        ...
        if saml_response_input and relay_state_input:
            ...
            aula_cookie_header = '; '.join([f'{c.name}={c.value}' for c in session.cookies if 'aula.dk' in c.domain or c.domain == ''])
            ...
            _LOGGER.debug(f"Aula SAML POST response status: {aula_response.status_code}")
            _LOGGER.debug(f"Aula SAML POST response headers: {dict(aula_response.headers)}")
            ...
            if 'Location' in aula_response.headers:
                ...
                _LOGGER.debug(f"Final Aula response status: {final_aula_response.status_code}")
                _LOGGER.debug(f"Final Aula response content length: {len(final_aula_response.text)}")
                ...
                if "login" in final_aula_response.url.lower():
                    ...
                    if final_aula_response.url == "https://login.aula.dk/auth/login.php?type=unilogin":
                        _LOGGER.warning("Redirected back to login page - authentication may have failed or requires additional steps")
                    else:
                        _LOGGER.info("Successfully completed authentication flow - redirected to Aula login page")
                else:
                    _LOGGER.info("Authentication completed - redirected to Aula dashboard or other page")
                    ...
                    if "portal" in final_aula_response.url.lower():
                        ...
                        portal_soup = BeautifulSoup(final_aula_response.text, 'html.parser')
                        csrf_token = None
                        ...
                        api_cookie_header = '; '.join([f'{c.name}={c.value}' for c in session.cookies if 'aula.dk' in c.domain or c.domain == ''])
                        if csrf_token:
                        ...
                        _LOGGER.debug(f"API response status: {api_response.status_code}")
                        _LOGGER.debug(f"API response headers: {dict(api_response.headers)}")
                            ...
                            try:
                                ...
                                if 'data' in profiles_data:
                                    ...
                                    if 'profiles' in data:
                                        ...
                                        _LOGGER.info(f"Number of profiles: {len(profiles)}")
                                        for profile in profiles:
                                            _LOGGER.info(f"Profile: {profile.get('name', 'Unknown')} - ID: {profile.get('id', 'Unknown')}")
                                    else:
                                        _LOGGER.debug(f"Available data keys: {list(data.keys())}")
                                else:
                                    _LOGGER.debug(f"Available top-level keys: {list(profiles_data.keys())}")

                            except Exception as e:
                                _LOGGER.error(f"Error parsing profiles JSON: {e}")
                                _LOGGER.debug(f"Raw response: {api_response.text[:500]}...")
                        else:
                            _LOGGER.error(f"API request failed with status {api_response.status_code}")
                            _LOGGER.debug(f"Response content: {api_response.text[:500]}...")
            else:
                _LOGGER.error("No final redirect found in Aula SAML response")
        else:
            _LOGGER.error("Could not find SAMLResponse or RelayState in after-post-broker response")
    else:
